    ...
    def edit_data():
        ...
        if selected_item >= 0 and data_baru:
            if dropdown_edit.get() == "Nama Ikan":
                ...
                jarak_tempuh[selected_item] = [kecepatan.strip(), waktu.strip(), jarak.strip()]
    ...
    def tambah_data():
        ...
        if dropdown_tambah.get() == "Jarak Tempuh":
            try:
                kecepatan, waktu, jarak = data_baru.split(',')
                jarak_tempuh = baca_file('jarak_tempuh.txt')
                jarak_tempuh.append([kecepatan.strip(), waktu.strip(), jarak.strip()])
                tulis_file('jarak_tempuh.txt', jarak_tempuh)
                messagebox.showinfo("Sukses", "Data Jarak Tempuh berhasil ditambahkan!")
            except ValueError:
                messagebox.showwarning("Peringatan", "Format salah! Masukkan data dalam format: kecepatan, waktu, jarak.")
            return
...
root = tk.Tk()
root.title("Manajemen Data Ikan")
root.configure(bg="#003366")

# Menampilkan header
tampilkan_header(root, "Manajemen Data Ikan")
...
# Tombol untuk tampilkan data
tombol_tampilkan = tk.Button(root, text="Tampilkan Data Ikan", command=halaman_tampilkan_data_ikan, bg="white", fg="black")
